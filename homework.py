...
def open_browser(browser_name):
    actual_result = inspect.currentframe().f_code.co_name.replace("_", " ").title() + f" [{browser_name}]"
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = inspect.currentframe().f_code.co_name.replace("_", " ").title() + f" [{page_url}]"
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result = inspect.currentframe().f_code.co_name.replace("_", " ").title() + f" [{page_url}, {button_text}]"
